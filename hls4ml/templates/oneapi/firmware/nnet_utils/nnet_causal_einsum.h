...
void read_causal_pipe(unsigned i, unsigned* write_ptrs, unsigned* ctx_cts, data_T* causal_buffer){
    ...
    ctx_cts[i] = std::min(ctx_cts[i] + 1, (unsigned)(CTX));
...
void read_stateless_pipe( 
    data_T data_vect_buffer[CONFIG_T::n_contract]){
                ...
                for(unsigned ctx = 0; ctx < CTX; ctx++){
                    ...
                    if(ctx < ctx_cts[i]){
                        ...
                        for(unsigned l1 = 0; l1 < L1; l1++){
                            ...
                            res_buffer[ctx_buff_offset + l1] = (res_T)tmp;
                            //res_buffer[ctx_buff_offset + l1] = tmp;
                        }
                    }else{
                        #pragma unroll
                        for(unsigned l1 = 0; l1 < L1; l1++){
                            res_buffer[ctx_buff_offset + l1] = res_T(0);//no need to pad with -INF since V mult has 0's in place 
                        }
                    }
                }
